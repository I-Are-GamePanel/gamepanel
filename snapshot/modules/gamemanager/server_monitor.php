...
echo "<table id='servermonitor' class='tablesorter' data-sortlist='[[0,0],[3,1]]'>".
	...
	foreach( $server_homes as $server_home )
	{
		  ...
		  )
		{
			...
			$query = "SELECT * FROM ogp_billing_orders WHERE home_id = " . $server_home['home_id'] . " AND end_date >= -2" ;
			$results = $db->resultQuery($query);
			if(!is_null($results[0]['end_date'])) 
			{
			//there is an end date
			if($results[0]['end_date'] > 0)
			{ 
			...
			if($results[0]['end_date'] == -1)
                        {
                        $expire_date = $results[0]['finish_date'];
			$expiration_dates = "<font color='yellow'>".  read_expire($expire_date) . "</font><a href='home.php?m=billing&p=cart'> Invoice</a>";
                        }
			// -2 its suspended, invoice still available
			if($results[0]['end_date'] == -2)
                        {
                        $expire_date = $results[0]['finish_date'];
			...
			if( $host_stat === 1)
			{
				...
				if($screen_running)
				{
					...
					if($server_xml->protocol == "gameq" and $server_xml->installer == 'steamcmd')
						$address = "<a href='steam://connect/" . $query_ip . ":" . $server_home['port'] . "'>Connect</a>";
			...
			$query = "SELECT * FROM ogp_billing_orders WHERE home_id = " . $server_home['home_id'] . " AND end_date > 0" ;
