 ...
 * Foundation, Inc., w59 Temple Place - Suite 330, Boston, MA  02111-1307, USA.
 *
 */
...
function exec_ogp_module()
{
	...
	if( ! $running )
	{
		...
		if ($retry >= 4)
		{
			echo "<p>Still Starting Server .. </br>SOME Servers take up to 5 minutes to start. </br>You can return to <a href='?m=gamemanager&amp;p=game_monitor&amp;home_id-mod_id-ip-port=$home_id-$mod_id-$ip-$port'>Return to Game Monitor</a></br>Be Patient ...</p>";

		}
		
		echo "<b>Starting Server ...  </b>";
		$retry++;
		$view->refresh("?m=gamemanager&amp;p=start&amp;refresh&amp;ip=$ip&amp;port=$port&amp;home_id=$home_id&amp;mod_id=$mod_id&amp;retry=".$retry,6);
		return;
	}
